#ДЗУрок2 ДЭДЛАЙН 09.11.2023 23 59
# ДЗ**:
...
# Например - Square side length: 5cm, perimeter: 20cm (значение подставляете из поля perimeter), area: 25cm (подсчитываете методом calculate_area).
...
# Например - Rectangle length: 5cm, width: 8cm, perimeter: 26cm (значение подставляете из поля perimeter), area: 40cm (подсчитываете методом calculate_area).
...
class Figure:
    ...
    def set_perimeter(self, new_perimeter):
        if new_perimeter <= 0:
            raise ValueError('Возможны только положительные значения для фигур')
        else:
            self.__perimiter = new_perimeter 
...
class Square(Figure):
    ...
    def __init__(self, side_length):
        ...
        # 10. В конструкторе класса Square должен высчитываться периметр квадрата, посредством вызова метода calculate_perimeter и возвращаемый результат метода должен задаваться атрибуту perimeter.
        self.set_perimeter(self.calculate_perimeter())
    ...
    # 13. В классе Square переопределить метод info, который бы распечатывал всю информацию о квадрате: 
    def info(self):
        print(f'Square side length: {self.__side_length}{self.unit}, perimeter: {self.get_perimeter()}{self.unit}, area: {self.calculate_area()}{self.unit}^2')
...
class Rectangle(Figure):
    ...
    # 19. В классе Rectangle переопределить метод info, который бы распечатывал всю информацию о прямоугольнике следующим образом: 
    def info(self):
            print(f'Rectangle length: {self.__length}{self.unit}, width:{self.__width}{self.unit}, perimeter: {self.get_perimeter()}{self.unit}, area: {self.calculate_area()}{self.unit}^2')

# 20. В исполняемом файле создать список из 2-х разных квадратов и 3-х разных прямоугольников
figure_list = Square(5), Square(7), Rectangle(2, 7), Rectangle(4, 1), Rectangle(10, 12)
